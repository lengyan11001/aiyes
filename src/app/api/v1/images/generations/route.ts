...
export async function POST(request: Request) {
  ...
  if (!isAllowedModel(body.model) || modelKind(body.model) !== "IMAGE") {
    return jsonError("Image model is not enabled.", 400, "model_not_allowed");
  }
  ...
  try {
    ...
    return NextResponse.json({ id: updated.id, object: "image.generation", status: updated.status, result: updated.result });
  } catch (error) {
